...
class MotorController():
    ...
    def __init__(self, config):
        ...
        channels = config.getlist('bldc', 'channels')
        init_pulse = config.getint('bldc', 'init_pulse')
        log.info("Attempting to add channel(s) %s at pulse %d" % (channels, init_pulse))
        for channel in channels:
            self.add_channel(channel, init_pulse)

        log.info("Initialized BLDC motor controller.")  

    def add_channel(self, raw_channel, init_pulse):
